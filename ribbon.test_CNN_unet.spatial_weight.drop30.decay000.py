...
def gen_tiles(save_dir,img_fn,seg_fn,hull_fn,tile_width,slice_nb,nb_tiles,drop):
...
def testNN(slice_fn,segment_fn,hull_fn,nb_tiles_in,drop,decay,verbose=False):
    ...
    save_dir=os.path.dirname('/home/rpizarro/histo/prediction/spatial_weight/drop_{0:03d}/decay_{1:03d}/set201/'.format(int(100*drop),int(decay)))
    ...
    # weights_load_fn=os.path.join(weights_dir,'weights.set094.epochs9500.FINAL.h5')
    weights_load_fn=os.path.join(weights_dir,'weights.set201.epochs20200.FINAL.h5')
    if weights_load_fn:
    ...
    tiles_norm,seg,coord,slice_shape=gen_tiles(save_dir,slice_fn,segment_fn,hull_fn,tile_width,slice_nb,nb_tiles,drop)
    nb_tiles=len(coord)
    ...
    fn='{0}_6x_concat_6x_whole.jpg.jac_idx_distribution.{1:04d}tiled.jac_idx{2:0.3f}.png'.format(slice_nb,nb_tiles,np.mean(jac_idx_val))
    plt.savefig(os.path.join(save_dir,fn))
    plt.close()

    Y_out_slice=Y_true_slice+3*Y_pred_slice
    fn='{0}_6x_concat_6x_whole.jpg.segmented.{1:04d}tiled.jac_idx{2:0.3f}.nii'.format(slice_nb,nb_tiles,np.mean(jac_idx_val))
...
def get_files(fnDir):
    ...
    with open(fnDir) as f:
        ...
        # all_files = [eval(i).strip().split(' ') for i in f]
...
decay=00
drop=0.3
test_fn = '/home/rpizarro/histo/weights/weights.spatial.weight/drop_{0:03d}/decay_{1:03d}/XValidFns/test.txt'.format(int(100*drop),int(decay))
...
for slice_fn,segment_fn in files:
    ...
    if hull_fn:
        print(hull_fn)
        testNN(slice_fn,segment_fn,hull_fn,nb_tiles,drop,decay,verbose=True)
