...
def gen_tile(img_fn,tile_width,nb_tiles):
    ...
    if shape[0]>tile_width:
        coord=[(0,0),(shape[0]-tile_width,0)]
    else:
        coord=[(0,0)]*2
    img_pad=np.zeros((596,560,3))+255
    ...
    tidx=0
    for x,y in coord:
        tile_tmp=img_pad[x:x+tile_width,y:y+tile_width]
        tile_pad=np.zeros(tiles.shape[1:])+255
        tile_pad[:tile_tmp.shape[0],:tile_tmp.shape[1]]=tile_tmp
        tiles_rgb[tidx,:,:,:]=tile_pad
        tile_norm=normalize(tile_pad)
        tiles[tidx,:,:,:]=tile_norm
            
        tidx+=1
    return tiles_rgb,tiles,coord,shape
...
def save_to_nii(data,fn):
    out_dir='/data/shmuel/shmuel1/rap/histo/prediction/subcortex/fold/'
...
def testNN(files,nb_tiles_in,verbose=False):
    ...
    for l,line in enumerate(files):
        try:
            ...
            tiles_rgb,tiles,coord,slice_shape=gen_tile(slice_fn,tile_width,nb_tiles)
            nb_tiles=len(coord)
            ...
            fn='snapshot_slide{0}_{1:04d}tiled.nii.gz'.format(slide_nb,nb_tiles)
            ...
            fn='snapshot_slide{0}-pred_segment_{1:04d}tiled.nii.gz'.format(slide_nb,nb_tiles)
            ...
            fn='snapshot_slide{0}-pred_edge_{1:04d}tiled.nii.gz'.format(slide_nb,nb_tiles)
...
data_path = "/data/shmuel/shmuel1/rap/histo/prediction/subcortex/fold/"
files = grab_files(data_path,"*.nii")
...
print(files)
testNN(files,nb_tiles,verbose=True)
