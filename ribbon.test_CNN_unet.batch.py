...
def gen_tiles(img_fn,seg_fn,tile_width,nb_tiles):
    img = nib.load(img_fn)
    hdr=img.header
    raw=hdr.structarr
    data = img.get_data()
...
def save_to_nii(data,fn):
    out_dir='/data/shmuel/shmuel1/rap/histo/prediction/subcortex/20180109_weights/'
...
def testNN(files,nb_tiles_in,verbose=False):
    ...
    weights_fn='./weights.gpu.label1.brown/weights.0840_of_1000_dice_coef_loss_-1.00.h5'
    ...
    for l,line in enumerate(files):
        try:
            ...
            tiles_rgb,tiles,seg,coord,slice_shape=gen_tiles(slice_fn,segment_fn,tile_width,nb_tiles)
            nb_tiles=len(coord)
            ...
            # fn='{0}-segmented_{1:04d}tiles.pred.nii.gz'.format(slice_nb,nb_tiles)
            ...
            # sh=Y_true_slice[:,:,0,0].shape
            # print(sh)
            # y_dc_true=np.reshape(Y_true_slice[:,:,0,0],(1,)+sh+(1,))
            # y_dc_pred=np.reshape(Y_pred_slice[:,:,0,0],(1,)+sh+(1,))
            # dc=dice_coef(y_dc_true,y_dc_true,y_dc_pred)
            # a=tf.Print(dc,[dc],message="This is slice dc: ")
            # b=a.eval()
            # dc_slice=float(b)
            # print('Dice coefficient slice : {0:0.3f}'.format(dc_slice))
            ...
            plt.savefig('/data/shmuel/shmuel1/rap/histo/prediction/subcortex/20180109_weights/'+fn)
            plt.close()
...
validation_files=[validation_files[i] for i in [0,2,4,5,10]]
