...
def get_sample_weights(y_true,drop,decay=50):
    # seg is (1,2560,2560,2)
    # if drop==0.0:
    #     return np.zeros(seg.shape)+1
    attention = np.zeros(y_true.shape)
    # f = 10.0
    b = 0.05
    # seg[0,:,:,0] is (X,Y)
    D0 = ndimage.distance_transform_cdt(y_true[0,:,:,0],metric='chessboard')
    D1 = ndimage.distance_transform_cdt(y_true[0,:,:,1],metric='chessboard')
    D=D0+D1
    w = np.exp(1-D/decay)
    w = w*(1-b)/np.exp(1) + b
    attention[0,:,:,0]=w
...
def weighted_categorical_crossentropy_fcn_loss(y_true, y_pred):
    ...
    # y_true_weights_maxed = get_sample_weights(y_true,decay)
...
def runNN(train_files,valid_files,test_files,drop,decay):
    ...
    weights_dir = os.path.dirname("../weights/weights.spatial.weight/drop_{0:03d}/decay_{1:03d}_500ep/".format(int(100*drop),int(decay)))
