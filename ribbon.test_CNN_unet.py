...
def get_coord(dim,tile_width):
    num_tiles = 2*int(np.ceil(float(dim)/tile_width))
...
def gen_tiles(img_fn,seg_fn,tile_width):
    img = nib.load(img_fn)
    hdr=img.header
    ...
    nb_tiles=len(coord_x)*len(coord_y)
    ...
    for x in coord_x:
	    ...
	    # tile_pad[:tile_tmp.shape[0],:tile_tmp.shape[1]]=tile_tmp
            ...
            tile_seg=segment(tile_norm,seg_tile)
...
def merge_tiles(slice_tiled):
    slice_shape=slice_tiled.shape[1:]
    slice=np.zeros(slice_shape)
    for x in range(slice_shape[0]):
        for y in range(slice_shape[1]):
            tile_sum = np.sum(slice_tiled[:,x,y])
            tile_nonzero=np.count_nonzero(slice_tiled[:,x,y])
            if tile_nonzero>0:
                slice[x,y]=float(tile_sum)/tile_nonzero
            else:
                slice[x,y]=0
    return slice


def merge_tile(slice_sum,slice_count,single_tile,x,y,tile_width):
...
def retile(tiles,coord,slice_shape,tile_width):
    ...
    print("we are retiling")
    print(tiles.shape)
    print(slice_shape)
    # slice_shape=tuple(np.asarray(slice_shape)+15)
    # typical size: (25,2666,2760)
    slice_tiled=np.zeros((nb_tiles,)+slice_shape[:-1])
    # print(slice_tiled)
    ...
    for x in coord_x:
        for y in coord_y:
            print(x,y)
            # print(tiles[:,:,tidx,0].shape)
            single_tile=tiles[:,:,tidx,0]
            slice_sum,slice_count=merge_tile(slice_sum,slice_count,single_tile,x,y,tile_width)
            slice_tiled[tidx,x:x+tile_width,y:y+tile_width]=tiles[:,:,tidx,0]
            # slice[x:x+tile_width,y:y+tile_width]=tiles[:,:,tidx,0]
            # print(slice[x:x+tile_width,y:y+tile_width].shape)
            # print(tiles[:,:,tidx,0].shape)
            tidx+=1
    # slice=merge_tiles(slice_tiled)
    print(np.unique(slice_count))
    # slice=slice_sum
    slice=np.true_divide(slice_sum,slice_count)
    # slice=np.true_divide(slice_tiled.sum(0),(slice_tiled!=0).sum(0))
...
def testNN(files,verbose=False):
    ...
    for l,line in enumerate(files):
        try:
            ...
            tiles_rgb,tiles,seg,coord,slice_shape=gen_tiles(slice_fn,segment_fn,tile_width)
            ...
            fn='{}-sliceindex_tiled.nii.gz'.format(slice_nb)
            ...
            fn='{}-segmented_tiled.nii.gz'.format(slice_nb)
...
testNN([validation_files[0]],verbose=True)
