...
def save_to_nii(data,fn):
    out_dir='/home/rpizarro/histo/prediction/drop/20180514_hull/drop_030_thresh'
...
def testNN(slice_fn,segment_fn,hull_fn,nb_tiles_in,drop,verbose=False):
    ...
    weights_fn=os.path.join(weights_dir,'weights.set019.epochs2000.FINAL.h5') 
    ...
    for n in range(nb_tiles):
        ...
        thresh=0.9999
        y_pred = np.around(y_prob > thresh)
    ...
    plt.savefig(os.path.join('/home/rpizarro/histo/prediction/drop/20180514_hull/drop_030_thresh',fn))
    plt.close()
