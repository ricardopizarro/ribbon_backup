...
def get_model(path,verbose=False):
    print(path)
    list_of_files = glob.glob(os.path.join(path,'model.set009*FINAL.h5'))
    if list_of_files:
        # print(list_of_files)
...
def testNN(slice_fn,segment_fn,hull_fn,nb_tiles_in,drop,decay,verbose=False):
    ...
    save_dir=os.path.dirname('/home/rpizarro/histo/prediction/spatial/drop_{0:03d}/decay_{1:03d}/epochs_500/set009/'.format(int(100*drop),int(decay)))
...
decay=00
drop=0.5
test_fn = '/home/rpizarro/histo/weights/spatial/drop_{0:03d}/decay_{1:03d}/epochs_500/XValidFns/test.txt'.format(int(100*drop),int(decay))
